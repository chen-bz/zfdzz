...
public class PayUtil {
   ...
   * @return
   */
  public static String getRedPacketNo() {
    // 自增8位数 00000001
    int num = (int) (Math.random() * 90) + 5;
    return "RED" + DatetimeUtil.formatDate(new Date(), DatetimeUtil.TIME_STAMP_PATTERN) + num;
  }
	...
	public static String getSign(Map<String, Object> params, String paternerKey) throws UnsupportedEncodingException {
		return SecureUtil.md5(createSign(params, false) + "&key=" + paternerKey).toUpperCase();
	}
	...
	public static String createSign(Map<String, Object> params, boolean encode) throws UnsupportedEncodingException {
		...
		boolean first = true;
		for (Object key : keys) {
			if (key == null || StrUtil.isEmpty(String.valueOf(params.get(key)))) // 参数为空不参与签名
				continue;
			if (first) {
				first = false;
			} else {
